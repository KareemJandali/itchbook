...
This design was written before the instrumentation existed. It was reviewed
adversarially before it was written down, and three findings corrected it; each
of the three would otherwise have produced a published number that was wrong.
Every figure below is measured on this machine or read from an artifact in
`validation/`; none is estimated.

The measurement **needed bare metal and has now been taken**: three boots on
2026-08-28, recorded in §11. The design and the corrections that preceded those
boots follow, in the order the corrections were found, and no attempt has been
made to rewrite them so that the work looks smooth. The results are in
...
**"t₀ → t₁ book updated (phase 10's number)."** This is a different number from
phase 10's, and it must never be printed as continuous with it. Phase 10's figure comes
from `tools/wire_to_book.cpp`, which is `receiver thread --ring--> book thread`:
two pinned threads and a 65,536-slot SPSC ring. In that harness *one*
arrival stamp is amortised across a `recvmmsg` batch. `tools/strategy.cpp` has
**zero** `std::thread` and no ring; it issues one `::recvfrom` per datagram, with
`seq.on_packet()` and `bk::apply()` inline in the same `poll()` loop. The program
differs, the queue differs, and the stamp granularity differs. So 12.8 measures
t₀→t₁ afresh, and phase 10's figure appears only in a separate column labelled
with the harness that produced it.

**"t₄ → t₅ match + ITCH publish."** The dominant term here is waiting, not
latency. A strategy order is filled only when a *historical* order at the same
price, added after it, is executed; `tools/strategy.cpp`'s own banner says so.
What t₄→t₅ therefore measures is how long the quote sat in the book waiting for
MSFT's own flow, which is market structure divided by `--multiplier`. At 1× that
is up to tens of seconds, and it saturates `Histogram`'s `UINT32_MAX` clamp as
well (1.193 s at the measured 3.599 cycles/ns). The interval is excluded from
every nanosecond total and every stacked bar, and reported separately in
**replay seconds**.

**"the full round trip."** Most orders have no round trip. ITCH names resting
orders only. On the recorded day (`validation/closed-loop-2019-12-30.json`)
1,180 of 1,200 orders were ever seen on the tape, and the remainder crossed, so
they can have no tape record at all. The round trip is therefore defined on a
selected subpopulation, and it is published together with the census that names
the exclusions.

**"the hops sum to the total."** When the hops are differences of stored stamps,
Σ(hops) = t_last − t_first **by telescoping, unconditionally**. The identity
survives a 10 µs cross-process clock offset and a stamp placed in the wrong
function. It survives an entirely un-instrumented region between two stamps as
well, because the time is still inside *some* hop and is merely labelled wrong.
A check that cannot fail is this repository's signature defect:
`scripts/closed-loop-check.sh` says *"this repo has produced a vacuous gate four
times, and three of those were a detector that could not run being scored as a
detector that found nothing."* §5 replaces it.
...
happened. Two constructions that do sum are given in §5.3.

---

## 1. Three findings that would each have produced a wrong number

The three findings below came out of an adversarial review of the design,
carried out before any code existed. Each one is verified against the source
rather than argued from principle.

### 1.1 The headline was anchored to a message with no causal role

In `tools/strategy.cpp:470-500` the quote block sits *after* the drain loop, and
it reads the book **at quote time**:
...
The input to the decision is therefore the *post-drain* book, that is, the last
message applied in the iteration. The message that crossed the counter
(`belief.applied - last_quote_at >= opt.quote_every`) only crossed a counter; its
content never reaches the price. There is a second effect. Because
`last_quote_at = belief.applied;` is assigned *at quote time*, after the drain,
the sampling stride is `max(quote_every, messages delivered in the wakeup that
crossed it)` and not `quote_every`. That quantity depends on the data, and it
inflates under load.
...
of unrelated work, to the write"* and calls the result tick-to-trade. A reader
who benchmarked that against a venue's published figure would be misled, and so
would 12.9 if it calibrated a latency model from it.

**Resolution.** The headline reaction-path number is **t₁′→t₃**, from the
post-drain decision to the write. **t₀→t₃ is reported separately and explicitly
as "arrival-to-write including queue drain"**, with the drain term named. The
realised stride is recorded per sample and published as a distribution instead of
being assumed.

### 1.2 p99.9 at the achievable sample size is mostly scheduler noise

Chain A yields roughly one sample per order, so of order 1,200 per run and
~12,000 over ten runs. The p99.9 of n = 12,000 is the twelfth largest sample.
`validation/cpu-jitter-baremetal.json` records, for cpu 7, 602 gaps over 10 µs in
20 s, which is **30.1 gaps/core/s**, with recorded-gap p50 **15,092 ns**, p99.9
**24,732 ns** and max **43,298 ns**. That is the *bare-metal* box. A chain that
spans a full drain occupies the core for tens of microseconds, so the expected
number of chains hit by a >10 µs gap is of order 1–7 out of the twelve that
*define* p99.9.

Between roughly 6% and 60% of the samples that set the published p99.9 are
therefore descheduling events recorded under a hop's label. The contaminated
fraction is a property of the machine and of the chain length, **not of n**, so
more runs only estimate the same contaminated quantity more precisely.

**Resolution.** The headline tail is **p99**. p99.9 is published only with a
**gap-overlap census** beside it, and two values are printed, one over all chains
and one over gap-free chains, together with the tagged count. A p99.9 without
that census is not printed at all.

### 1.2.1 The gap-overlap census, built — and there is no watchdog

The design above asked for a co-pinned watchdog. No good version of one exists.
Pinned to the **same** core, it contends with the thread it exists to observe and
disturbs what it measures. Pinned to a **different** core, it observes the wrong
core, because gaps are per-core.

The kernel answers the question directly instead. Over any window,

    off_cpu = wall_elapsed − thread_cpu_elapsed

is the time the thread was not running, attributed exactly to that window, with
no second thread and no contention. It is the same quantity `cpu_jitter` already
...
The clock sees a deschedule exactly, and it almost never reports one that did not
happen. The threshold is **5 µs**, well above the instrument's 130 ns noise floor
and well below the 15,092 ns median gap it exists to catch.

**The clock is read outside the interval**, immediately before t₁′ and
immediately after t₃, so that its ~145 ns cost does not inflate a headline hop
which may be single-digit microseconds on bare metal. One consequence follows:
the CPU window strictly contains the wall window, so a chain that never stopped
reads slightly **below** zero, typically −626 ns here. That value is the
bracket's own zero and is reported as such, and the bias is toward **missing** a
...
CPU time. off_cpu is ~0 for every chain, including the twelve slowest, which ran
27–40 µs against a median of 8.8 µs and were **running the whole time**. That
tail comes from the loopback `write()` syscall path, not from descheduling. The
hop is also misnamed: the encode is the 90 ns in `t1′→t2`, and `t2→t3` is
overwhelmingly the write.

Two things follow for the boot. Measurements must be taken on the **release**
build, and the sanitised build is used only for the correctness gates. The
per-hop names in the results document must also say `write` where they currently
say `encode, frame, write`.

### 1.3 Every chain-A sample is taken at the loop's most favourable instant

The UDP drain runs to `EAGAIN`, and the quote block executes after it, at most
once per `poll()` iteration. So t₁′→t₂→t₃ is *always* measured with an empty
receive queue and with the book's lines just touched by the drain. The TCP socket
has also been idle for the whole drain. Jittering the quote cadence does not
help, because the bias is in **loop phase** and not in stride, and every stride
lands at the same phase.

**Resolution.** The results document states this as a condition on the numbers:
all chain-A hops after t₁′ are conditioned on an empty receive queue. The
drain-burst length distribution is published beside them. A separately labelled
`--quote-in-handler` arm quotes from inside `FeedHandler::on_message` and so
samples at a random loop phase, and it is run as a comparison. That arm changes
the program under test, so it is a variant and never the default, and the
four-arm gate is re-run against it.
...
published 1,222,930 messages in 27,218 data packets, which is **44.9 messages per
datagram**. Folded into "match + ITCH publish", the packing delay is a term
nobody can explain whose value is a function of `--mtu`. Split out and named, it
has a control, and halving `--mtu` should roughly halve it. That sweep is run.
...
The costs are close enough to make no difference, so the decision rests on three
other grounds. *One epoch, one scale*: `calibrate_cycles_per_ns()` is run
independently in each process and two calibrations of the same counter differ by
~1e-4 relative, so nanoseconds would not sum across the process boundary even
with perfectly synchronised counters. *The stamps join the machinery that causes
the outliers*: `session.tick()`, the heartbeats, `kTickPeriodNs` and
`--ack-delay-ms` all already run on `CLOCK_MONOTONIC`. *Several stamps already
exist* as `wall_now_ns()`.
...
threshold was set against a ~5.3 µs wire-to-book p50, and 12.8's hops are an
order of magnitude smaller by construction. The bound must not exceed **10% of
the smallest reported cross-process hop's p50**, and both numbers and their ratio
go in the artifact.
...
catastrophic across cores. It substitutes 0 with no counter and biases p50 down.
It also destroys the only live evidence that the clock claim is wrong.
Intra-process negatives **abort the run**, since they are a harness bug and not a
clock finding. Cross-process negatives are counted and their worst is printed
beside every percentile, and any negative larger than the `tsc_offset` bound
**vetoes** the run.
...
- **t₀→t₁, all messages**: every applied message, with `msg_index_in_datagram`
  ...
  cache effect, and it is neither.
- **t₀→t₁, trigger messages only**: published as a second column. If the two
  ...
  disagreement is stated instead of averaged.
...
- **Excluded**: chains whose paired `rdtscp` stamps disagree on CPU, which means
  pinning silently failed, and chains whose arena slot overflowed. Anything with
  a negative intra-process hop aborts the run instead.
...
sum. One counter does not exist today, *distinct orders with at least one maker
fill*, and it is added; without it only bounds are available.
...
which selects for busy stretches of tape, exactly where chain A is worst. The
headline is therefore reported over **all** orders, with the filled-subset column
printed beside it, so that the selection effect is visible instead of inferred.
...
**(a) Instrumented coverage.** There is a stamp at the top and at the bottom of
the strategy's `while (true)`, and per iteration the sum of every hop stamped
inside it is taken. `iteration_wall − Σ(hops)` is un-instrumented time, published
as a coverage fraction with p50 and p99. This catches an entire region nobody is
timing, which is the failure the telescoping identity structurally cannot see. A
decomposition whose coverage is 60% is not a decomposition, and the report says
so.
...
made and its exit code was unreachable. The input to that gate was a constant. A
...
Alongside, in a table **explicitly marked non-additive**, three quantities
appear: per-hop p50/p99 with n and order-statistic confidence intervals; the
named residual `Σ p99(hop) − p99(total)`; and the argmax-hop frequency within C,
that is, *"in N% of the slowest chains the dominant hop was the packing delay"*.
The last of those is what "explain the hop" means operationally.
...
well controlled while every hop is inflated by contention. That is a check which
cannot fail. **Different physical packages** is also a veto: cross-socket TSC
...
it exists to flag, which is the same shape as the kernel-drop gate.
...
offset bounded under 52 ns, which clears the existing `BOUND_NS = 1000.0` by a
...
timing**. These sit in the path a real order takes, on every order, so they are
not measurement artefacts. They were fixed first, then measured and gated.
...
Executed back) both did `std::vector<uint8_t> frame(len + 3);`. That is a malloc
and a free per message in both directions, with a zero-fill of bytes about to be
overwritten in between. Phase 10 found the same value-initialisation mistake in
...
memmove per write, and it is now a consumed-offset cursor. The token used
`snprintf` with a format string and a locale walk to produce fourteen fixed
bytes; the replacement is byte-identical over the tested range, and the four-arm
gate checks that at no extra cost, because the token is what the exchange echoes
back.
...
**Before the boot.** Build every binary **static**, because the live session has
no compiler and may have no network: `g++ -O2 -std=c++20 -static -I include -o
...
the repo) and a save script on the stick, since a live session loses everything
at reboot. Record the commit SHA and verify the tree is clean.
...
choose CPUs for you. Instead it prints the machine's real topology, marks which
cores are isolated, and stops, because phase 10's numbering must not be inherited
and `nproc` reports the calling shell's affinity mask rather than the machine (16
CPUs in sysfs here against `nproc`'s 13). The same CPU twice is vetoed, as are
SMT siblings and different physical packages. Every tool's exit code is checked
before the file that tool was told to write is read. `cpu_jitter` runs on exactly
the two chosen cores and is treated as the decisive gate; `tsc_offset` runs on
exactly the chosen pair, and the harness takes the *larger* of the method's
resolution and its estimate as the bound, so the figure quoted beside a hop is
never optimistic. A short dry run comes next, and the long one is refused unless
**every hop took samples and chain B joined at least one fill**, because a hop
with zero samples scored as a pass is the failure that already happened twice in
12.7. Afterwards both probes are re-run and the clocksource is re-read, and a
machine that moved under the run fails it.

It also records the use of non-isolated cores when the machine has isolated ones.
Here cpus 13/14 are isolated and measurably quieter than 2/6, at 23 and 42 gaps
over 100 µs against 170 and 167. That is a five-fold difference nobody would have
...
them** and requires the report to refuse each one, because a live run cannot test
a refusal. A live run produces whatever it produces, and a silently wrong join
still looks like a table of plausible latencies. The planted defects are stamps
out of order, an exchange record naming an order that was never sent, the same
token twice, a fill whose two independently-counted ordinals disagree, a chain
that stopped running mid-hop (which the census must **tag**, the test that it can
fire at all), and a run with no census (where p99.9 must not be printed).

It also reads a fixture the C++ test writes and parses it with the report's own
...
tail-conditional mean. `scripts/phase12-8-results.py` generates
`docs/phase12-8-results.md`. Both are wired into `verify-local`'s
tracked-files-only gate, so a stale figure or table fails the build instead of
...
There were three, all on the same day and the same machine. The first ran at the
powersave governor and is kept as evidence rather than as results. The second
produced the numbers. The third was taken only to tie those numbers to a commit,
and it found that the gate meant to establish that could never have passed.
...
The third boot's headline came back 8,139 ns against the second's 8,169, which is
0.4% apart, on a different pair of cores and from a separately built kit. That is
the strongest statement in the phase, and no single run could have made it.
...
tree, which left the phase's strongest claim resting on a number a reader could
not check. That is exactly the defect this phase had already fixed once, in
...
median of 38,435, which is **2.0%**, against 13–18% under WSL2. Eight of twelve
...
against ~1,300, and the headline 38,435 against 8,845, **uniformly, across hops
that share no code**. A uniform slowdown across unrelated paths is the clock the
code was running at, and not a code finding. A live session defaults to powersave
and nothing set it otherwise.

The harness **printed the governor and quoted the numbers anyway**. That is the
defect: a latency measured at an unspecified CPU frequency is not a latency. The
harness now sets the performance governor itself and verifies that the change
took, since writing to sysfs can fail silently. Anything else is treated as a
reason not to quote, the absent case included, because WSL2 has no cpufreq at all
and is precisely the machine that check exists to flag.

### 11.3 Two smaller things the boot found

**The auto-picker chose cpu0.** That was legal: on this hardware cpu0's sibling
is cpu8, so 0 and 1 are distinct physical cores and every veto passed. It was
still wrong, because cpu0 is where Linux points device interrupts by default,
which makes it the busiest core on an otherwise idle machine. It is now chosen last. The live boot also had no `isolcpus`, so there were no isolated cores to
prefer.
...
run; the ingest globbed all seventeen and pooled them as one experiment. Those
are deliberately different configurations, since `--multiplier 500` and `2000`
exist *because* they should differ, so pooling them asks whether runs set up to
differ do differ. The repeats are pooled alone now, and every other configuration
is reported on its own. The ingest also wrote its working tree to `/tmp`, which
is wiped between WSL invocations here, so the raw material vanished between two
commands.

### 11.4 What the next boot needs

Nothing but a re-run, the same single command. The governor is set by the harness
now, cpu0 is avoided, and everything else about the first boot was clean. The run
takes ~25 minutes and the numbers from it will be at a known, verified clock.
