...
emitting matcher. The plan requires this order and gives the reason: the
topology determines what the gateway is **for**, and a gateway written first
would encode a topology nobody chose.*

Nothing here is a measurement. Everything here is a decision, its cost, and the
test that would prove it wrong.
...
before a line of protocol code is written, and only the first is answered in the
build plan:
...
Question 1 is where v1.0 was wrong. Questions 2–4 are consequences that only
appear once question 1 is answered correctly, and getting any of them wrong
produces a system that runs, publishes plausible bytes, and measures nothing.
...
entered*. Your book is not that book: ITCH publishes executions as **facts**, and
a matcher fed only adds has no crossing events to consume, so its book drifts
away from the real one from the first execution onward. Adds that were resting
orders in reality arrive at a diverged book and **cross**. From there the emitted
stream diverges for reasons that have nothing to do with any strategy, and
§12.4's backtester-modelling-error table — the deliverable that makes phase 12 an
experiment rather than a demo — is dominated by that artifact.

The failure is not "the simulation is imperfect." It is that the error term you
are trying to measure gets swamped by an error you introduced yourself, and the
...
phase-9 path you already trust." That is correct and it is not sufficient, because
taken literally it makes the strategy's orders **unfillable**.

Consider a historical execution: `E` says order reference *R* traded *N* shares.
Applied as state, order *R* loses *N* shares and your resting order is untouched.
Do that for every execution in the day and your maker never trades — the
aggressor that would have hit you is not in the feed as an order, only as its
consequence on someone else.
...
the aggressor takes your shares first — which is exactly the fill the queue model
in phases 6 and 11 was *estimating*, now actually executed by the matcher against
a real book.

One consequence must be stated because it is a divergence and not a bug: when
the aggressor consumes your shares, the historical order *R* ends the day holding
more shares than history records. The historical `E` is **consumed by the
aggressor path and never also applied as state** — applying both would
double-count the trade. The size of that divergence is bounded by the shares your
orders absorbed, and it is attributable order by order, which is what makes
§12.4's table meaningful rather than mysterious.
...
This is not a convenience. Queue position is the entire subject of phases 6 and
11, and it only exists if your order and the historical orders it competes with
sit in one queue. Two books — a "market" book the matcher consults and a "mine"
book it owns — would put your order in a queue of one and hand it a fill rate
nothing in reality supports.

The two sources of mutation are kept distinguishable at the order level, not by
holding separate books:
...
Both paths mutate the same book through the same code. That matters for a reason
beyond tidiness: the emitted ITCH is generated from book mutations, so a mutation
that bypassed the matcher would be a mutation the published feed never described,
and the strategy's own reconstruction would silently diverge from the exchange's.
...
strategy's book at a single price level and keeps running.
...
- **Strategy references occupy the high half of the space** (bit 63 set).
  Historical NASDAQ references have never been observed in that range, but "never
  observed" is a claim about a sample, so —
...
references are exchange-side) is a separate mapping and is not a substitute for
the partition. A token collision is a protocol error the gateway rejects; a
reference collision is a silent book corruption downstream.

---

## 6. Two clocks, deliberately

The system runs on two independent time bases and conflating them produces
numbers that look reasonable and mean nothing.

**Replay time** is the market's clock. It is driven by the historical feed's own
nanosecond timestamps, advanced by the replayer, and it is what every emitted
ITCH message is stamped with — including messages the matcher generates for
strategy orders. A day replayed at 10× real time still produces a stream whose
internal timestamps describe the original session, because that is what a
consumer's book arithmetic depends on.

**Wall clock** is the measurement clock. Tick-to-trade — §12.3's headline — is
the real elapsed time your code takes: packet arrival at the strategy socket to
OUCH enter on the wire. It has nothing to do with replay time and must never be
...
2. **The strategy's decisions are functions of replay time; its latency is
   ...
   not machine time. Feeding it wall clock during a 50× replay would collapse its
   horizon by a factor of fifty and change the strategy rather than the load.
...
1. **Terminate a SoupBinTCP session** — login, sequenced data, heartbeats both
   ways, end of session. Heartbeat death is wired to the kill switch: a dead
   session flattens, and that is a test with a cancel count in it, not a hope.
2. **Validate OUCH and answer every inbound message exactly once** — `A`, `U`,
   `C`, `E` or `J` with a reason code. Unknown token, bad price, wrong state are
...
4. **Apply the risk layer before the matcher, not after** — `risk/kill_switch.hpp`
   in line, price collar against the current book, max order rate, max position.
   Risk that runs after matching is accounting, not risk.

What the gateway is explicitly **not** for: it does not see historical flow. The
replayer does not speak OUCH and never enters a session. That asymmetry is the
§3 decision made structural — historical participants are state and events, not
counterparties with sessions.
...
- **No market impact model, in either lane.** Deliberately: an unvalidated impact
  model would add an error term with no ground truth to check it against, which
  is worse than a stated absence.
- **The claim "real flow trades against my venue" is given up.** What is kept is
  an experiment whose disagreements are attributable. That is the trade §12.4
  exists to exploit.
...
- **P1 — the zero-strategy identity.** With no strategy orders entered, the book
  ...
  right decomposition, replaying a day through it and re-deriving the book is a
  round trip that must close exactly. **If P1 fails, §3 is wrong and the topology
  goes back to the drawing board rather than getting a tolerance.**
...
- **P4 — the backtester is optimistic.** Live fills through the loop will be
  *fewer* than the phase-11 `mbo` lane predicts for the same day and strategy,
  ...
  a real matcher does. Direction predicted; magnitude not.
- **P5 — determinism holds under load.** A fixed inbound script produces
  ...
  speed changes the output, something is reading wall clock that should be
  reading replay time (§6).
...
- **OUCH version.** 4.2 is simpler and is the plan's leaning, but the field table
  gets built from the spec before the version is fixed in writing.
- **Whether the aggressor carries the historical execution's price or the
  book's.** They differ for `C` (executed with price), which is precisely the
  message that says the trade printed away from the resting price. Needs the
  message-level detail work of §12.1 before it can be settled honestly.
...
original feed. That claim is P1, it is testable before any strategy exists, and it
is testable on data already in hand.

**Build P1 first.** Before the OUCH field tables, before SoupBinTCP, before the
gateway. It needs the replayer, the matcher's existing `submit`, and the phase-3
book — all of which exist — and it decides whether the rest of phase 12 is built
on a topology that works or one that merely sounds better than v1.0's.
