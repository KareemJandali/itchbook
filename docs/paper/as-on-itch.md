...
> **The scope is met on counts and not on composition.** The pre-registration names a *kind* per slot — MSFT, a mid-cap, an ETF — and evaluated here are GOOG, MSFT, STOR. Specifically QQQ is named in the plan and absent from this paper, with no calibration artifact and therefore no refusal on record either: not evaluated, and not declined. §8.5 carries it as open work and `docs/paper/review/` carries the reason it went unstated for as long as it did.
...
36 cells is **not** a sample of 36. The four lanes of a symbol-day are four scorings of one feed on one stock on one day; the replication unit here is 9 symbol-days, arguably 3 symbols, and no count in this document should be read as anything wider.
...
Only the **naive** lane fitted, so there is no cross-lane spread to report — k = 483.9 is one number from one world, not a range. The other three lanes are refusals and are counted as such in §8.4; this symbol is not evaluated in §7.
...
**Every k above is one iteration, not a fixed point.** λ̂'s denominator is exposure, and the exposure is generated by running the A-S maker itself — which needs a k before one has been measured. All 4 calibrations here bootstrap from γ = 0.005 and k = 200, recorded in each artifact as `gamma` and `k_assumed`. Where an order rests, and therefore which depth buckets accumulate exposure, is conditional on that guess; the map has not been iterated to convergence and no convergence check exists. The distance travelled in one step is not uniform — median fitted k against the bootstrap: GOOG 16.3×; AMD 2.4×; MSFT 1.6×; STOR 1.0× — so the symbol furthest from its own starting point is **GOOG**, which is also the smallest measured k here and therefore the symbol carrying §8.3's wide-floor case. A second pass under the measured parameters is the check that settles it and it has not been run.

Depth is integrated as the mid moves (§6), which is correct for exposure and is **not** the conditional distribution A-S consumes. λ(δ) enters the spread formula as the fill rate for an order *placed* at δ; much of the exposure at large δ here comes instead from orders the mid walked away from, which are adversely selected moments. The two are different conditionings and the bias between them is not shown to be neutral. Splitting the estimate into placed-at-depth and drifted-to-depth would test it; that split does not exist today.
...
3 symbol(s) — GOOG, MSFT, STOR — over 3 evaluation day(s), with 2019-08-30 held out as the calibration day. Quote size 100, modelled latency 0 ns. γ is swept over 0.001, 0.005, 0.02, 0.1; the tables below fix γ = 0.02 and the sweep itself is the figure. That γ is **chosen by rule and not by outcome** — `three_arm_cells` takes the middle of the swept positive grid, whatever the grid is and whatever the numbers do — and §8.1 recounts the headline at every swept γ so the choice can be checked rather than trusted.
...
**That count does not depend on the γ the tables fix.** Recomputed at every swept γ (0.001, 0.005, 0.02, 0.1), A-S captures less half-spread than the touch maker in 36 of 36 cells at all 4 — only the number of outright *negative* cells moves, over 14 to 21. γ = 0.02 is the middle of the swept grid by rule (§7), and on this axis the choice does not carry the finding.

Every count out of 36 above shares the §1 caveat: 36 cells are four lanes over 9 symbol-days on 3 stocks, and the lanes within a symbol-day are not independent draws.
...
1. **Latency sensitivity — graded on 2 points, which is not a sweep.** 2 modelled latencies (0 ns, 500,000 ns) are committed and P4 is graded against them in §7.5. Two endpoints establish a direction and not a shape, and the pre-registration asked for a ladder: `tools/latency_sweep.cpp` runs one, but on the open-loop `backtest.hpp` path with no A-S arm, so it cannot grade this prediction. The ladder here is N `as_experiment` runs under the `validation/as-experiment*.json` names this generator already globs, and it has not been run. The verdict is not restated here; one copy of a graded prediction is the most this document keeps.
2. **The outside reader — closed, with its limits stated.** §4 was reviewed at `5ee2b7b` by a reader who wrote neither the paper nor the code, verdict *major revision*. The reader was **not blind** — they helped sketch the experimental design that became build-plan 11.3, disclosed by the reviewer in the report's first paragraph. They raised 13 findings and 3 questions, of which Q2, M5 bear on §4, and **did not contest the band-over-worlds construction itself**. That is evidence a competent reader with the code in front of them did not object to §4; it is not evidence that §4 is right, and the difference is the reason this item records the shape of the review and not a tick. Report and point-by-point response: `docs/paper/review/`.
...
5. **The referee's open findings.** 8 of 16 items from the review at `5ee2b7b` are accepted and unfixed: **M1** recalibrate once under the measured parameters and report how far k moves; then split exposure into placed-at-depth and drifted-to-depth; **M2** calibrate QQQ on the held-out day and either evaluate it or put it on the §8.4 refusal list with its reason; **M3** emit max drawdown, a sampled inventory path and the 100 ms / 10 s markouts into the run record, then report them; **M4** run the latency ladder as N as_experiment artifacts and annotate P4's verdict as the bare, correlated majority it is; **m2** one sentence in §8.4 connecting STOR's weak fits to its P4 failure, and the 24-of-24 recount without STOR; **m4** state the fee schedule in the paper, and record FeeSchedule::name in as-experiment.json; **m6** emit the run parameters into the artifact and generate the table; the inventory cap is absent because max_position is 0; **m7** refit k on one evaluation day, labelled post hoc, in the same pass as M1. None is a disagreement; most need the experiment re-run against the licensed feed. Dispositions for all 16, including the ones already fixed, are in `docs/paper/review/2026-08-31-response.md`.
