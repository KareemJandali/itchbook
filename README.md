...
> drop. Phase 11, the paper, is **seven of seven**: its results are **run and
> graded**, and the last item — a review by someone who did not write it —
> closed on 2026-08-31. The referee's verdict was **major revision, findings
> expected to survive**; the report, a point-by-point response and a
> machine-readable index are committed under `docs/paper/review/`, and **eight
> of the sixteen items are accepted and still open**, listed in the paper's own
> §8.5. The phase is done; the paper is not.
...
[**Avellaneda–Stoikov on NASDAQ TotalView-ITCH, with fill-model uncertainty
bands**](docs/paper/as-on-itch.md) — calibrating the fill-intensity curve
λ(δ) = A·e^(−kδ) from the project's own queue-position-resolved fills instead of
assuming A and k, and evaluating an inventory-aware maker against a naive one
through four closed-loop fill models.

**Run.** Three symbols (GOOG, MSFT, STOR) over three evaluation days, calibrated
on a held-out 2019-08-30, four fill models — 36 symbol-day-lane cells, and 288
runs in `validation/as-experiment.json`. That meets the pre-registered scope,
and the seven predictions committed before the harness existed are graded in
print: **four kept, one falsified, one mixed, one not triggered.**

**The headline is a negative result, which is why it was worth pre-registering.**
Inventory-aware quoting does *not* lose less than naive symmetric quoting. A-S
captures less half-spread per share in **36 of 36 cells** and goes outright
negative in 16, while holding a median **75% less inventory** — it buys inventory
control with adverse selection, which is the reverse of what was predicted (P2,
falsified).

The three-arm design is what makes that decomposable. Running A-S with the
inventory skew switched off separates the **spread choice** from the **skew**,
and they point in opposite directions: the skew is a real, isolated win — with
it A-S holds less inventory than the naive maker on 3 of 3 symbols, without it
*more* on 3 of 3 — while the spread choice is where the money goes, and it is
the half the pre-registration was not looking at. A two-arm comparison would
have bundled both and attributed everything to inventory awareness.

**The mechanism is the tick.** A-S's spread has an inventory-free floor,
`(2/γ)·ln(1 + γ/k)`, and once k is *measured* rather than assumed that floor is
a number the venue may not be able to express. Measured k spans 11.9 to 483.9
across symbols and lanes. Where the floor clears half a tick the captured edge
stays positive; where it lands **below half a tick** — below the smallest
increment Reg NMS lets anyone quote — there is no price that expresses what the
model is asking for, and the edge goes negative. That test predicts the sign on
**3 of 3** symbols, and it is falsifiable on the next one. An implementation
that assumes k never discovers that its own spread formula wants a price the
venue does not have; **assuming k is what hides this, and measuring it is what
shows it.**

Carried openly rather than in a footnote: §4's band-over-worlds methodology has
not been reviewed by anyone who did not write it, and §8.3's test is carried by
three symbols of which one sits on the wide-floor side of the line — with n=1
there, "the floor fits the grid" and "this particular symbol" are not separated
by this sample. Both are in [§8.5](docs/paper/as-on-itch.md).

Every number and verdict below §6 is generated by `scripts/paper-report.py` from
committed artifacts; none is typed. With no artifact it emits an explicit "no
results" block naming what is missing, and the predictions stay marked ungraded
— the state this section was in until the data landed. There is no path by which
a number reaches that page without an artifact behind it, and CI checks the
paper, its figures and its rendered HTML on every push, including a run over
synthetic feeds that proves the results generator works independently of whether
