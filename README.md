...
**Methodology complete, results pending data**, and the paper is built so that
the second half of that sentence cannot quietly disappear. Its results sections
are generated by `scripts/paper-report.py` from committed artifacts; with no
artifact it emits an explicit "no results" block naming what is missing, and the
seven predictions committed before the harness existed stay marked ungraded.
There is no path by which a number reaches that page without an artifact behind
it, and CI checks the paper, its figures, and its rendered HTML on every push —
including a run over synthetic feeds that proves the results generator works
before the day real data arrives.
